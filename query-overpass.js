...
var x = Number(argv.lonlat.split(',')[0]);
var y = Number(argv.lonlat.split(',')[1]);
var point = turf.point([x, y]);
var buffer = turf.buffer(point, argv.radius, 'kilometers');
var bbox = turf.bbox(buffer);
var overpassBbox = [bbox[0], bbox[2], bbox[1], bbox[3]].toString();
...
overpass(query, function(error, data) {
    ...
    console.log(json2csv({'data': d, 'fields': ['id', 'lon', 'lat', 'place', 'name', 'name:en', 'name:zh', 'wikipedia', 'wikidata']}));
